...
def analyze(filepath):
    ...
    # Rimuovi artefatti iniziali (primo picco troppo vicino)
    if len(peaks) >= 3:
    ...
    if beat_count >= 2:
        ...
        if has_dual:
            ...
            if n_pairs >= 1:
                ...
                if len(s1_times) >= 2:
                    s1_ints = np.diff(s1_times)
                    avg_int = np.mean(s1_ints)
                    bpm = int(60.0 / avg_int) if avg_int > 0 else 0
                    if len(s1_ints) > 1:
                        std_i = np.std(s1_ints)
                        cv = std_i / (avg_int + 1e-9)
                        reg = max(0.0, 1.0 - cv)
                        nsc = min(1.0, len(s1_times)/20.0)
                        conf = round(0.5 + 0.3*reg + 0.2*nsc, 2)
                    else:
                        conf = 0.5
                if len(s1_times) >= 3:
                    sdnn = float(np.std(s1_ints))
                    diff_sq = np.square(np.diff(s1_ints))
                    rmssd = float(np.sqrt(np.mean(diff_sq))) if len(diff_sq) > 0 else 0.0
                    pnn50 = float(np.mean(np.abs(np.diff(s1_ints)*1000 > 50.0)) * 100.0)
                    hrv = {'sdnn_sec': round(sdnn,4), 'rmssd_sec': round(rmssd,4), 'pnn50_percent': round(pnn50,2), 'mean_hr_sec': round(float(np.mean(s1_ints)),4)}
                s1_s2_class = {'s1_count': int(len(s1_times)), 's2_count': int(len(s2_times)), 's1_avg_amplitude': float(np.mean(s1_amps)) if len(s1_amps) else 0.0, 's2_avg_amplitude': float(np.mean(s2_amps)) if len(s2_amps) else 0.0}
                display_peak_times = s1_times.tolist()
                display_amplitudes = s1_amps.tolist()
                beat_count = len(s1_times)
        else:
            s1_times = times_arr
            intervals_s1 = intervals_all
            avg_int = np.mean(intervals_s1)
            bpm = int(60.0 / avg_int) if avg_int > 0 else 0
            if len(intervals_s1) > 1:
                std_i = np.std(intervals_s1)
                cv = std_i / (avg_int + 1e-9)
                reg = max(0.0, 1.0 - cv)
                nsc = min(1.0, len(s1_times)/20.0)
                conf = round(0.5 + 0.3*reg + 0.2*nsc, 2)
            ...
            if len(intervals_s1) >= 2:
                ...
                rmssd = float(np.sqrt(np.mean(diff_sq))) if len(diff_sq) > 0 else 0.0
                pnn50 = float(np.mean(np.abs(np.diff(intervals_s1)*1000 > 50.0)) * 100.0)
    ...
    envelope_data = {'times': t_env[::step].tolist(), 'values': env_norm[::step].tolist()}

    return {'duration': round(duration,2), 'bpm': bpm, 'beat_count': beat_count, 'confidence': conf,
            'peak_times': display_peak_times, 'amplitudes': display_amplitudes, 'sample_rate': sr,
            's1_s2': s1_s2_class, 'hrv': hrv, 'envelope': envelope_data, 'filter_low': 20, 'filter_high': 150}
