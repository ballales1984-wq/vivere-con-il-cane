...
def analyze(filepath):
    ...
    # Rilevamento picchi - parametri sensibili
    ...
    # Rimozione artefatti iniziali (manteniamo, ma con soglie adattate)
    if len(peaks) >= 3:
    ...
    # Pulizia outlier (solo se >= 7 picchi, conservativa)
    if len(peaks) >= 7:
    ...
    if beat_count >= 2:
        ...
        if has_dual:
            ...
            if n_pairs >= 1:
                ...
                if len(s1_times_arr) >= 2:
                    ...
                    if len(s1_ints) > 1:
                        ...
                        reg = max(0.0, 1.0 - cv)
                        nsc = min(1.0, len(s1_times_arr)/20.0)
                ...
                s1_s2_class = {'s1_count': int(len(s1_times_arr)), 's2_count': int(len(s2_times_list)), 's1_avg_amplitude': float(np.mean(s1_amps_arr)) if len(s1_amps_arr) > 0 else 0.0, 's2_avg_amplitude': float(np.mean(s2_amps_arr)) if len(s2_amps_arr) > 0 else 0.0}
            ...
            if len(intervals_s1) > 1:
                ...
                cv = std_i / (avg_int + 1e-9)
                reg = max(0.0, 1.0 - cv)
                nsc = min(1.0, len(times_arr)/20.0)
                conf = round(0.5 + 0.3*reg + 0.2*nsc, 2)
            else:
                conf = 0.5
